...
public class DonateHandler extends AbstractHandler
{
  ...
  public Object execute(ExecutionEvent event) throws ExecutionException
  {
    ...
    try
    {
      ...
      if ("true".equals(PropertiesUtil.getProperty("org.eclipse.oomph.setup.dontate.navigation", "false"))) //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      {
        style |= IWorkbenchBrowserSupport.NAVIGATION_BAR;
      }

      if ("true".equals(PropertiesUtil.getProperty("org.eclipse.oomph.setup.dontate.location", "false"))) //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      {
