...
public class SetupArchiver implements IApplication
{
  public Object start(IApplicationContext context)
  {
          ...
          public InputStream createInputStream(URI uri, Map<?, ?> options) throws IOException
          {
            ...
            try
            {
              ...
              else
              {
                ...
                throw new IOException(MessageFormat.format(Messages.SetupArchiver_InconsistentResults_exception, uri));
              }
            }
    ...
    else
    {
      ...
      {
        throw new IORuntimeException(MessageFormat.format(Messages.SetupArchiver_CouldNotSetTimestamp_exception, temp));
      }
      ...
      try
      {
        ...
        for (Enumeration<? extends ZipEntry> entries = zipFile.entries(); entries.hasMoreElements();)
        {
          ...
          System.out.println(MessageFormat.format(Messages.SetupArchiver_PreviouslyMirrored_message, uri, archiveEntry));
        }
      }
      catch (IOException ex)
      {
        if (!file.delete())
        {
          throw new IORuntimeException(MessageFormat.format(Messages.SetupArchiver_CouldNotDeleteBadVersion_exception, file));
        }
    ...
    for (Resource resource : resourceSet.getResources())
    {
      ...
      {
        System.err.println(MessageFormat.format(Messages.SetupArchiver_FailedToLoad_message, normalizedURI));
    ...
    if (!hasEcoreFailures)
    {
      ...
      for (Resource resource : resourceSet.getResources())
      {
        ...
        if (normalizedURI.query() == null && ("http".equals(scheme) || "https".equals(scheme))) //$NON-NLS-1$ //$NON-NLS-2$
        {
          ...
          System.out.println(MessageFormat.format(Messages.SetupArchiver_Mirroring_message, normalizedURI));
          ...
          {
            System.err.println(MessageFormat.format(Messages.SetupArchiver_FailedToLoadProperly_message, normalizedURI));
          }
          else
          {
            try
            {
              ...
              if (after - before > 0)
              {
                System.err.println(MessageFormat.format(Messages.SetupArchiver_Changed_message, normalizedURI));
              }
            }
            catch (IOException ex)
            {
              System.err.println(MessageFormat.format(Messages.SetupArchiver_FailedToSave_message, normalizedURI));
              ex.printStackTrace();
            }
          }
        }
        ...
        {
          System.out.println(MessageFormat.format(Messages.SetupArchiver_Ignoring_message, normalizedURI));
        }
      }
    ...
    if (lastModified != finalLastModified)
    {
      if (OS.INSTANCE.isWin())
      {
        ...
        {
          System.err.println(MessageFormat.format(Messages.SetupArchiver_CouldNotDelete_message, file));
        }
      }
      ...
      if (lastModified == 0)
      {
        if (isDamaged(file))
        {
          System.err.println(MessageFormat.format(Messages.SetupArchiver_ArchiveDamaged_message, file));
          file.delete();
        }
        else
        {
          System.out.println(MessageFormat.format(Messages.SetupArchiver_SuccessfullyCreated_message, file));
        }
      }
      else if (isDamaged(temp))
      {
        System.err.println(MessageFormat.format(Messages.SetupArchiver_ArchiveDamagedKeepOld_message, temp));
        temp.delete();
      }
      else
      {
        ...
        {
          System.err.println(MessageFormat.format(Messages.SetupArchiver_CouldNotBackUp_message, backup));
        }

        if (temp.renameTo(file))
        {
          System.out.println(MessageFormat.format(Messages.SetupArchiver_SuccessfulUpdates_message, file));
        }
        else
        {
          System.err.println(MessageFormat.format(Messages.SetupArchiver_CouldNotRename_message, temp, file));
        }
      }
    }
    else
    {
      System.out.println(MessageFormat.format(Messages.SetupArchiver_NoUpdates_message, file));
      if (!temp.delete())
      {
        System.err.println(MessageFormat.format(Messages.SetupArchiver_CouldNotDelete_message, temp));
      }
    }
  ...
  private void printDiagnostics(List<Resource.Diagnostic> diagnostics)
  {
    ...
    {
      System.err.println(MessageFormat.format(Messages.SetupArchiver_DiagnosticError_message, diagnostic.getMessage(), diagnostic.getLine(),
