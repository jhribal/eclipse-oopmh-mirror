      ...
      try
      {
        ...
        try
        {
          LOOP: for (int retry = 0; retry <= MAX_RETRY; ++retry)
          {
            if (retry > 0)
            {
              log(IStatus.WARNING, MessageFormat.format(Messages.CachingRepositoryManager_LoadingFailed_message, location, retry), failure);
            }
  ...
  public static class Artifact extends ArtifactRepositoryManager
  {
    ...
    static final class BetterMirrorSelector extends MirrorSelector implements SynchronousProvisioningListener
    {
      ...
      public synchronized URI getMirrorLocation(URI inputLocation, IProgressMonitor monitor)
      {
        ...
        try
        {
          ...
          if (previousActivity != null)
          {
            ...
            monitor.subTask(MessageFormat.format(Messages.CachingRepositoryManager_RepeatedDownload_task, artifactURI));
          }
        ...
        {
          CachingRepositoryManager.log(IStatus.ERROR, MessageFormat.format(Messages.CachingRepositoryManager_NonRelative_message, inputLocation, location), e);
        }
      ...
      private String getStats(MirrorActivity mirrorActivity)
      {
        ...
        String message = MessageFormat.format(Messages.CachingRepositoryManager_Speed_message, successes, mirrorActivity.getLocation(),
            numberFormat.format(mirrorActivity.getSpeed() / 1000));
        if (failures > 0)
        {
          message += " " + MessageFormat.format(Messages.CachingRepositoryManager_Failure_message, failures); //$NON-NLS-1$
        }
